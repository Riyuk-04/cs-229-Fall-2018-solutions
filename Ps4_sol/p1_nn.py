...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    y = forward_softmax(x)
    temp = np.diag(y) - np.outer(y,y)
    result = temp.dot(grad_outputs)
    return result
    # *** END CODE HERE ***
...
def forward_convolution2(conv_W, conv_b, data):
    """
    Compute the output from a convolutional layer given the weights and data.
    ...
    conv_channels, conv_width, conv_height = conv_W.shape
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    Del_W = np.zeros(conv_W.shape)
    Del_b = np.zeros(conv_b.shape)
    Del_data = np.zeros(data.shape)

    conv_channels,_,conv_width,conv_height = conv_W.shape

    Del_W = forward_convolution2(output_grad,Del_b,data)
    padded_output_grad = np.pad(output_grad,((0,0),(conv_width - 1,conv_height - 1),(conv_width - 1,conv_height - 1)),'constant')
    Del_data = forward_convolution(conv_W,Del_b,padded_output_grad)
    Del_b = np.sum(output_grad,axis = (1,2))
    return Del_W,Del_b,Del_data 
    ...
    for x in range(0, input_width, pool_width):
        for y in range(0, input_height, pool_height):
            for c in range(input_channels):
                index =np.argmax(data[c, x:(x + pool_width), y:(y + pool_height)])
                output[c, x:(x + pool_width), y:(y + pool_height)].flat[index] = output_grad[c, x // pool_width, y // pool_height]
...
def backward_linear(weights, bias, data, output_grad):
    ...
    # *** START CODE HERE ***
    D_b = output_grad
    D_data = output_grad.dot(weights.T)
    D_weights = np.outer(data,output_grad)
    return D_weights,D_b,D_data
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params):
    ...
    del_y = backward_cross_entropy_loss(y, labels)
    del_logits = backward_softmax(logits, del_y)
    del_W2, del_b2, del_flattend = backward_linear(params['W2'], params['b2'], flattened, del_logits)
    del_first_after_relu = del_flattend.reshape(first_after_relu.shape)
    del_first_max_pool = backward_relu(first_max_pool, del_first_after_relu)
    del_first_convolution = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, del_first_max_pool)
    del_W1, del_b1, del_data = backward_convolution(params['W1'], params['b1'], data, del_first_convolution)
    
    dictionary = {'W1': del_W1, 'b1': del_b1, 'W2': del_W2, 'b2': del_b2}
